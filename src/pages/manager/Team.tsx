...
function AddTeammateForm({ managerId, onDone }: { managerId: string; onDone: () => void }) {
  ...
  return (
    <AppShell title="Team" subtitle={`${team.length} people report to you.`}>
      <div className="space-y-4">
        {showAdd ? (
          <AddTeammateForm managerId={managerId} onDone={() => setShowAdd(false)} />
        ) : (
            ...
            return (
              <Card key={member.id} className="group relative p-5 transition-shadow hover:shadow-glow">
                <button
                  onClick={(e) => {
                    e.preventDefault()
                    if (confirm(`Remove ${member.name} from your team?`)) removeEmployee(member.id)
                  }}
